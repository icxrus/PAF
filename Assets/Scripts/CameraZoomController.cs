...
public class CameraZoomController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        inputProvider.FindActionMap("Player Movement").FindAction("Zoom").performed += cntxt => ZoomYAxis = cntxt.ReadValue<float>();
        inputProvider.FindActionMap("Player Movement").FindAction("Zoom").canceled += cntxt => ZoomYAxis = 0f;
    }
    ...
    private void UpdateZoomLevel() //updates all the rigs to be correct to the zoom level
    {
        ...
        cmCamera.m_Orbits[1].m_Radius = currentMiddleRigRadius;
        cmCamera.m_Orbits[0].m_Height = cmCamera.m_Orbits[1].m_Radius;
        cmCamera.m_Orbits[2].m_Height = -cmCamera.m_Orbits[1].m_Radius;
    }
